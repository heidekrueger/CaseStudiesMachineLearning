...
\section{SQN: A Stochastic Quasi-Newton Method}
  ...
  \begin{frame}{Stochastic Newton Method}
    \begin{columns}[T]
      ...
      \begin{column}{.5\textwidth}
        ...
        $$\min \mathbb E \left [f(\omega, \xi)\right]$$
        \uncover<3>{
        $$\omega^{k+1}:= \omega^{k}-\alpha_k \alert{\nabla^2 \hat F(\omega^{k})^{-1}} \alert{\nabla \hat F(\omega^{k})} $$
          with
          $$\alert{\nabla^2 \hat F(\omega^{k})} := \frac{1}{b_H}\sum_{i\in \mathcal S_{H,k}}f(\omega, x_i, z_i)$$
          where $\mathcal S_{H,k} \subset [N], \quad b:=|\mathcal S_{H,k}| \ll N$\\\alert{"Mini Batch"}
         }
      \end{column}
    \end{columns}
  \end{frame}
  ...
  \begin{frame}
    ...
    \resizebox{0.8\linewidth}{!}{\input{'EEG Fixed Subset Objective vs. Accessed Data Points.tikz'}}
    \center{Performance on EEG Dataset, Problem size: $69550 \times 600$\\
    \tiny{Armijo-stepsizes, Further SQN-parameters: $L=10$, $M=5$}}
  \end{frame}
