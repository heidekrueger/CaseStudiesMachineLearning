# -*- coding: utf-8 -*-
"""
Created on Tue Jul  7 19:00:33 2015
...
def compute_0sr1(f, grad_f, x0, X, z, **options):
    """
    Stochastic Zero-memory Symmetric Rank 1 Proximal method for function of
    form F = f + lambda * h, with f smooth and h the l1-norm (i.e. ||x||_1)
    ...
    z : target (2D array: nsamples x 1)
    ...
    options.setdefault('tau', 0.0001)
    options.setdefault('tau_min', 1e-12)
    options.setdefault('tau_max', 1e7)
    ...
    for k in range(1, int(options['max_iter'])+1): # make while or itercount later
        ...
        temp_x_new = compute_proximal(u_H, u_B, d_H, d_B, grad_f(x_new, X_b, z_b), x_new, **options)
