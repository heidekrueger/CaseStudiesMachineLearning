...
\section{PCA}
Input: A set of $K$ vectors $X_i \in \R^n$, $i \in \{1,\dots K\}$ with is normalized, i.e. \ba \frac{1}{ K} \sum_{i = 1}^K X_i = 0.\ea
...
\ba
\max \left\{ \sum_{i=1}^K  \norm{\Phi X_i} - \sum_{k=1}^m \lambda_k (\norm{\phi_k}_2 - 1) \right\}
\ea
...
\ba
&\frac{\partial}{\partial \phi_k} \sum_{k=1}^m \phi_k^T \Sigma \phi_k - \lambda_k( \norm{\phi_k}_2 - 1 ) = 0 \\
